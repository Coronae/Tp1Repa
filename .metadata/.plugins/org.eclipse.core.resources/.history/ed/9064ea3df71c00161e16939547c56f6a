...
public abstract class RestJson {
	...
	public static List<Account> getListAccount() throws Exception {
		...
		List<Account> listAccount = (List<Account>) jsonUnmarshaller.unmarshal(jsonStream2, Account.class).getValue();
		connection.disconnect();
	...
	public static List<Transac> getListTransac() throws Exception {
		...
		List<Transac> listTransac = (List<Transac>) jsonUnmarshaller.unmarshal(jsonStream2, Transac.class).getValue();
		connection.disconnect();
		for (Transac element : listTransac)
			System.out.println(element.toString());
		return listTransac;
